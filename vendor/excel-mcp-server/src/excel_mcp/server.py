...
from excel_mcp.python_runner import execute_python_code
...
def run_python(
    ...
    """
    Execute custom Python code with openpyxl helpers.

    Helpers available inside the code:
    - data: optional input payload
    - Workbook, load_workbook, Path, json
    - resolve_path(filepath): resolve using the same filepath rules as this server
    - save_workbook(workbook, filepath): save workbook using server path policy
    - read_json(filepath), write_json(filepath, payload)
    - output: assign any return value to this variable

    The tool returns JSON with `output` and captured `stdout`.
    """
    try:
        result = execute_python_code(code=code, resolve_path=get_excel_path, data=data)
        import json
