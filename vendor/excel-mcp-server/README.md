...
- 🐍 **Custom Python Escape Hatch**: Run targeted Python/openpyxl snippets when built-in tools are not enough
...
This fork adds a `run_python` tool that keeps the original server structure but gives you a controlled escape hatch for one-off automation.

- The tool exposes `Workbook`, `load_workbook`, `Path`, `json`, and an optional `data` payload.
- Use `resolve_path(filepath)` to resolve a file path with the same rules as the server.
- Use `save_workbook(workbook, filepath)` to save an `openpyxl` workbook safely through the same path policy.
- The tool returns JSON with:
  - `output`: whatever value your code assigns to `output`
  - `stdout`: anything printed during execution
...
ws.title = "Summary"
ws["A1"] = "Hello"
save_workbook(wb, "reports/demo.xlsx")
output = {"status": "ok"}
```
