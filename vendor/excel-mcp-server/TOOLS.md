...
Execute custom Python code with helpers aligned to the server's file path rules.
...
- `data`: Optional JSON-like payload exposed as `data` inside the script
- Available helpers inside the script:
  - `Workbook`, `load_workbook`, `Path`, `json`
  - `resolve_path(filepath)`
  - `save_workbook(workbook, filepath)`
  - `read_json(filepath)`
  - `write_json(filepath, payload, indent=2)`
  - `output` variable for returning structured results
- Returns: JSON string containing `output` and captured `stdout`
